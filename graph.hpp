...
public:
  ...
  template<typename T>
  T * dealloc_vertex_subset(T * array) {
    delete array;
  }
  ...
  void load_directed(std::string path, VertexId vertices) {
    ...
    //if(partition_id==0)
      //  for(int i_i=0;i_i<partitions;i_i++)
        //    for(int t_i=0;t_i<threads;t_i++)
          //      printf("tuned_chunk_dense[%d][%d]:%u\n",i_i,t_i,tuned_chunks_dense[i_i][t_i]);
